...
interface FeedbackContentStepProps {
  ...
  onFeedbackRestartRequested: () => void; // tipo função com retorno nulo
  onFeedbackSent: () => void;
}
...
export function FeedbackContentStep ({
  ...
  const [screenshot, setScreenshot] = useState<string | null>(null) // Recebe a screenshot
  const [comment, setComment] = useState('') // comentário (feedback do cliente)
  const [isSendingFeedback, setIsSendingFeedback] = useState(false)

  const feedbackTypeInfo = feedbackTypes[feedbackType];

  async function handleSubmit(event: FormEvent)  { // FormEvent é tipagem própria para eventos de formulário
      ...
      <div>
        <header>
          ...
          <span className="text-xl leading-6 flex items-center gap-2 justify-center">
            <img src={feedbackTypeInfo.image.url} alt={feedbackTypeInfo.image.alt} className="w-6 h-6"/>
